...
%    \draw (3, 9.25) node[left]{\footnotesize\emph{idx == 0}} ;
  ...
  % TxD
    ...
    \draw[->] (6.75, 1) -- ++ (0, -1) ;
    \draw (6.75, 0.5) node[fill=white, draw] {\texttt{CAN Tx}};
  % RxD
    \draw[<-] (8.75, 1) -- ++ (0, -1) ;
  ...
  %  Rx Buffers
    ...
    \draw (13, 5.25) node {\emph{Reception FIFO}};
  \end{tikzpicture}
...
{\bf Receiving messages.} The MCP2515 \emph{CAN Protocol Engine} transmits all correct frames to the \emph{reception filters}. By default, they are configured as pass-all, see \refSectionPage{primaryFilters} and  \refSectionPage{secondaryFilters} for configuring them. Messages that pass the filters are stored in the \emph{Reception FIFO}. Its depth is not configurable -- it is always 2-message. The message interrupt service routine transfers the messages from \emph{Reception FIFO} to the \emph{Driver Receive Buffer}. The size of the \emph{Driver Receive Buffer} is 32 by default -- see \refSubsectionPage{driverReceiveBufferSize} for changing the default value. Three user methods are available:
\begin{itemize}
...
{\bf Sequentiality.} The \texttt{ACAN2515} driver and the configuration of the \texttt{MCP2515} controller can ensure sequentiality of data messages\footnote{Sequentiality means that if an user program calls \texttt{tryToSend} first for a message $M_1$ and then for a message $M_2$, the message $M_1$ will be always retrieved by \texttt{receive} or \texttt{dispatchReceivedMessage} before the message $M_2$.}. The reception part of the \texttt{ACAN2515} ensures sequentiality. The emission part of the \texttt{ACAN2515} ensures sequentiality \emph{for a given transmit buffer}: if an user program calls \texttt{tryToSend} first for a message $M_1$ specifying the $B_i$ buffer and then for a message $M_2$ specifying the same buffer, the driver ensures that $M_1$ will be sent on the CAN bus before $M_2$. However, if $M_2$ specifies an other buffer, there is no guarantee that $M_1$ will appear on the bus before $M_2$. 
...
void setup () {
  ...
  ACAN2515Settings settings (125 * 1000) ; // 125 kbit/s
...
{\bf Line 9.} This is the third step, configuration of the \texttt{can} driver with \texttt{settings} values. You cannot change the \texttt{can} name -- see \refSectionPage{driverInstances}. The driver is configured for being able to send any (standard / extended, data / remote) frame, and to receive all (standard / extended, data / remote) frames. If you want to define reception filters, see \refSectionPage{primaryFilters} and  \refSectionPage{secondaryFilters}.

{\bf Lines 10 to 15.} Last step: the configuration of the \texttt{can} driver returns an error code, stored in the \texttt{errorCode} constant. It has the value $0$ if all is ok -- see \refSubsectionPage{errorCodeMethodBegin}.
...
You have several connection possibilities:
\begin{itemize}
  \item using \emph{software SPI} or \emph{hardware SPI};
  \item using the \texttt{MCP2515} interrupt signal or not.
\end{itemize}

For efficiency, it is recommended to use \emph{hardware SPI} and the \texttt{MCP2515} interrupt signal.

\subsection{Software SPI}

Software SPI requires four pins that provide digital input and output. We name these pins \texttt{MCP2515\_CS}, \texttt{MCP2515\_SCK}\texttt{MCP2515\_SI} and \texttt{MCP2515\_SO}, as shown in \refFigure{}{figureSoftwareSPI}.
...
\begin{figure}[!ht]
  ...
  \begin{tikzpicture}[line cap=round, line join=round, >=stealth]
  %--- Microcontroller
    \draw[very thick, fill=lightgray!25] (-1.5, 0) -- ++(5.5, 0)  -- ++(0, 2.5) -- ++ (-5.5, 0) ;
    \draw (0, 1.25) node {\bf\emph{Microcontroller}};
  %--- MCP2515
    \draw[very thick, fill=lightgray!25] (12, 0) -- ++(-4, 0)  -- ++(0, 2.5) -- ++ (4, 0) ;
  ...
  %--- SCK
    ...
    \draw (4, 1.5) node[left] {\tt MCP2515\_SCK};
  %--- SI
    \draw[->, thick] (4, 1) -- ++ (4, 0) ;
    \draw (8, 1) node[right] {\tt SI};
    \draw (4, 1) node[left] {\tt MCP2515\_SI};
  %--- SO
    \draw[<-, thick] (4, .5) -- ++ (4, 0) ;
    \draw (8, .5) node[right] {\tt S0};
    \draw (4, .5) node[left] {\tt MCP2515\_SO};
  \end{tikzpicture}
  \caption{Software SPI}
  \labelFigure{figureSoftwareSPI}
\end{figure}

The four argument constructor of \texttt{ACAN2515} class specifies the for pins used for handling software SPI:
{ \small\begin{lstlisting}[language=c++]
...
\subsection{Hardware SPI}

Hardware SPI requires you use dedicaced pins of your microcontroller (\refFigure{}{figureHardwareSPI}). You can use alternate pins (see below), and if your microcontroller supports several hardware SPIs, you can select any of them. However, you should provide one digital port for handling $\overline{\tt CS}$ signal.
...
The two argument constructor of \texttt{ACAN2515} class specifies the for pins used for handling hardware SPI:
{ \small\begin{lstlisting}[language=c++]
static const byte MCP2515_CS  = 20 ; // CS input of MCP2515 
...
If your microcontroller provides \texttt{SPI2}, \texttt{SPI3}, … you can select the one you want:
{ \small\begin{lstlisting}[language=c++]
ACAN2515 can (MCP2515_CS, SPI2) ;
\end{lstlisting}}

The \texttt{begin} function of \texttt{ACAN2515} library configures the selected SPI with a speed of 10 Mbit/s.

\subsubsection{Using alternate pins on Teensy 3.x}
...
For example, on a Teensy 3.5, I want to use \texttt{SPI0} with these alternate pins\footnote{See \url{https://www.pjrc.com/teensy/pinout.html}}:
\begin{figure}[!ht]
  \small
  \centering
  \begin{tikzpicture}[line cap=round, line join=round, >=stealth]
...
\subsection{\texttt{MCP2515} interrupt pin is not connected}

This design saves one pin. This is not recommended because incoming messages can be lost, the \texttt{MCP2515} receive buffer is 2-message wide.
\begin{figure}[!ht]
  \small
  \centering
  \begin{tikzpicture}[line cap=round, line join=round, >=stealth]
  %--- Microcontroller
    \draw[very thick, fill=lightgray!25] (-1.5, 0) -- ++(5.5, 0)  -- ++(0, 1) -- ++ (-5.5, 0) ;
    \draw (0, .5) node {\bf\emph{Microcontroller}};
  %--- MCP2515
    \draw[very thick, fill=lightgray!25] (12, 0) -- ++(-4, 0)  -- ++(0, 1) -- ++ (4, 0) ;
    \draw (10.5, .5) node {\bf\emph{MCP2515}};
  %--- INT
    \draw[<-, thick] (6, .5) -- ++ (2, 0) ;
    \draw (8, .5) node[right] {$\overline{\tt INT}$};
    \draw (6, .5) node[left] {\tt nc};
  \end{tikzpicture}
  \caption{the $\overline{\tt INT}$ pin of the \texttt{MPC2515} is not used}
  \labelFigure{figureNoInterrupt}
\end{figure}

You have two possibilities: calling the \texttt{polling} function in \texttt{loop}, or calling the \texttt{isr} function in a periodic interrupt function.


\subsubsection{Calling the \texttt{polling} function in \texttt{loop}}

Your \texttt{loop} function should call the \texttt{polling} function as often as possible:
{ \small\begin{lstlisting}[language=c++]
void loop () {
  can.polling () ;
  ...
...
\subsubsection{Calling the \texttt{isr} function in a periodic interrupt function}

You define a periodic interrupt, and call \texttt{can.isr} function in its attached interrupt service routine:
{ \small\begin{lstlisting}[language=c++]
void periodicISR () {
  can.isr () ;
}
...
void setup () {
  ...
  const uint32_t errorCode = can.begin (settings) ;
  ...
//--- This should be done AFTER the call of can.begin
...
\subsection{Using \texttt{MCP2515} interrupt pin}

This design requires one interrupt pin. This is recommended for not losting incoming messages.
\begin{figure}[!ht]
  \small
  \centering
  \begin{tikzpicture}[line cap=round, line join=round, >=stealth]
  ...
  %--- INT
    \draw[<-, thick] (4, .5) -- ++ (4, 0) ;
    \draw (8, .5) node[right] {$\overline{\tt INT}$};
    \draw (4, .5) node[left] {\tt MCP2515\_INT};
  \end{tikzpicture}
  \caption{the $\overline{\tt INT}$ pin of the \texttt{MPC2515} is used}
  \labelFigure{figureInterrupt}
\end{figure}

It is up to you to set up the interrupt handling. First, you declare the interrupt pin number (you can select any pin that have interrupt capability):
{ \small\begin{lstlisting}[language=c++]
static const byte MCP2515_IRQ = 37 ; // INT output of MCP2515
\end{lstlisting}}

Then, you write the interrupt service routine; it just calls the \texttt{isr} method of your \texttt{can} object:
{ \small\begin{lstlisting}[language=c++]
void canISR () {
  can.isr () ;
}
...
Finally, in the \texttt{setup} function, you configure the pin as external interrupt, and attach the interrupt service routine; this should be done before calling \texttt{can.begin}:
{ \small\begin{lstlisting}[language=c++]
void setup () {
  ...
...
The \texttt{idx} field of the message is an 8-bit unsigned integer:
\begin{itemize}
  \item the bits 5 to 0 define the transmit buffer (0 $\rightarrow$ transmit buffer 0, 1 $\rightarrow$ transmit buffer 1, 2 $\rightarrow$ transmit buffer 2, any other value $\rightarrow$ transmit buffer 0);
  \item the bits 7 and 6 define the priority of the \texttt{TXBi} corresponding buffer\footnote{See Microchip document \texttt{DS20001801H}, section 3.2 page 15.}.
\end{itemize}

The default value of the \texttt{idx} field is zero: the message is sent throught \texttt{TXB0} with a priority equal to 0.
...
{ \small\begin{lstlisting}[language=c++]
const uint32_t s = can.transmitBufferSize () ;
\end{lstlisting}}
...
{ \small\begin{lstlisting}[language=c++]
const uint32_t n = can.transmitBufferCount () ;
\end{lstlisting}}
...
{ \small\begin{lstlisting}[language=c++]
const uint32_t max = can.transmitBufferPeakCount () ;
\end{lstlisting}}
...
%
%{ \small\begin{lstlisting}[language=c++]
%ACAN2515Settings settings (125 * 1000) ;
...
{\bf Note. } This section applies only to {\bf remote} frames. For sending data frames, see \refSectionPage{sendingFrames}.
...
Note the driver receive buffer may overflow, if messages are not retrieved (by calls of \texttt{receive} or \texttt{dispatchReceivedMessage} methods). If an overflow occurs, further calls of \texttt{can.receiveBufferPeakCount ()} return \texttt{can.receiveBufferSize ()+1}.
...
The \texttt{mConfiguration} property of the \texttt{settings} variable defines the FlexCAN module configuration -- see \refTableau {configFlexCAN}. By default, its value is \texttt{ACAN2515Settings::k12\_12\_Filters}.

You can easily override the default configuration:
{ \small\begin{lstlisting}[language=c++]
void setup () {
  ACAN2515Settings settings (125 * 1000) ;
  settings.mConfiguration = ACAN2515Settings::k14_18_Filters ;
  const uint32_t errorCode = can.begin (settings) ; // No receive filter
  ...
}
\end{lstlisting}}
...
%
%In the following example, all network messages are retrieved by calling \texttt{can.receive}. Then, the \textbf{if} ... \textbf{else if} dispatches useful messages to \texttt{handle\_myMessage\_0}, ... functions.
%
%{ \small\begin{lstlisting}[language=c++]
%void setup () {
%  ACAN2515Settings settings (125 * 1000) ;
...
%void loop () {
%  CANMessage message ;
%  if (can.receive (message)) { // All network frames are retrieved
%    if (!message.rtr && message.ext && (message.id == 0x123456)) {
%      handle_myMessage_0 (message) ; // Extended data frame, id is 0x123456
%    }else if (!message.rtr && !message.ext && (message.id == 0x234)) {
%      handle_myMessage_1 (message) ;  // Standard data frame, id is 0x234
...
{ \small\begin{lstlisting}[language=c++]
void setup () {
  ACAN2515Settings settings (1 * 1000 * 1000) ; // 1 Mbit/s
...
void setup () {
  Serial.begin (9600) ;
  ACAN2515Settings settings (842 * 1000) ; // 842 kbit/s
...
void setup () {
  Serial.begin (9600) ;
  ACAN2515Settings settings (842 * 1000, 100) ; // 842 kbit/s, max distance is 100 ppm
  Serial.print ("mBitConfigurationClosedToWishedRate: ") ;
...
void setup () {
  Serial.begin (9600) ;
  ACAN2515Settings settings (500 * 1000, 0) ; // 500 kbit/s, max distance is 0 ppm
  Serial.print ("mBitConfigurationClosedToWishedRate: ") ;
  Serial.println (settings.mBitConfigurationClosedToWishedRate) ; // 1 (--> is true)
  Serial.print ("actual bit rate: ") ;
...
void setup () {
  Serial.begin (9600) ;
  ACAN2515Settings settings (440 * 1000) ; // 440 kbit/s 
  Serial.print ("mBitConfigurationClosedToWishedRate: ") ;
  Serial.println (settings.mBitConfigurationClosedToWishedRate) ; // 0 (--> is false)
  Serial.print ("actual bit rate: ") ;
...
void setup () {
  Serial.begin (9600) ;
  ACAN2515Settings settings (500 * 1000) ; // 500 kbit/s
  Serial.print ("mBitConfigurationClosedToWishedRate: ") ;
  Serial.println (settings.mBitConfigurationClosedToWishedRate) ; // 1 (--> is true)
