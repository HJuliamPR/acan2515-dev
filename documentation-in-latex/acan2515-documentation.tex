...
\begin{figure}[!ht]
  ...
  %--- MCP2515 controller
    ...
    \draw (5.25, 3.5) node[below] {\bf\emph{MCP2515 CAN Controller}};
  % Flèches
...
{\bf Receiving messages.} The FlexCAN \emph{CAN Protocol Engine} transmits all correct frames to the \emph{reception filters}. By default, they are configured as pass-all, see \refSectionPage{primaryFilters} and  \refSectionPage{secondaryFilters} for configuring them. Messages that pass the filters are stored in the \emph{Reception FIFO}. Its depth is not configurable -- it is always 2-message. The message interrupt service routine transfers the messages from \emph{Reception FIFO} to the \emph{Driver Receive Buffer}. The size of the \emph{Driver Receive Buffer} is 32 by default -- see \refSubsectionPage{driverReceiveBufferSize} for changing the default value. Three user methods are available:
\begin{itemize}
...
\begin{itemize}
  ...
  \item on sending messages, it is used for selecting the transmit buffer.
\end{itemize}
...
\section{Connecting a \texttt{MCP2515}}
...
\subsection{The \texttt{tryToSend} method}

You select the transmit buffer by setting the \texttt{idx} property of a message:
\begin{itemize}
  \item \texttt{0} (default value) the message is sent trought the driver transmit buffer 0 by \texttt{TXB0};
  \item \texttt{1} the message is sent trought the driver transmit buffer 1 by \texttt{TXB1};
  \item \texttt{2} the message is sent trought the driver transmit buffer 2 by \texttt{TXB2};
  \item any other value is interpreted as 0 the message is sent trought the driver transmit buffer 0 by \texttt{TXB0}.
\end{itemize}

Call the method \texttt{tryToSend} for sending frames; it returns:
\begin{itemize}
