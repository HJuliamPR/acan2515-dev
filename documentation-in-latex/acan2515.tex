...
\title{\bf \Huge{ACAN2515 library for Arduino\\Version 1.1.0}}
...
So, when \texttt{transmitBufferPeakCount($i$)} returns a value lower or equal to \texttt{transmitBufferSize ($i$)}, it means that calls to \texttt{tryToSend} have allways returned \texttt{true}, and no overflow occurs on driver transmit buffer $i$.
