...
The filters of the \texttt{MCP2515} are quite primitive, and it is recommended to read the Microchip documentation \texttt{DS20001801H}, section 4.5 page 33. The \refFigurePage{}{figureFiltres2515} shows the \texttt{MCP2515} acceptance filter registers.
...
\texttt{ACAN2515Settings} class has two properties \texttt{RXM0} and \texttt{RXM1}. They are set to $0$ by default. These properties are written to the corresponding \texttt{MCP2515} registers by the \texttt{ACAN2515::begin} method, so, by default, the \texttt{MCP2515} receives all CAN bus messages.
...
You can set the \texttt{mRolloverEnable} property of your \texttt{ACAN2515Settings} object to \texttt{true} (it is \texttt{false} by défault). Doing that, if a new message is received as \texttt{RXB0} is full, the new message is transferred to \texttt{RXB1}. If \texttt{RXB1} is full the new message is lost.
...
The \texttt{MCP2515} filters cannot be disabled, so all mask registers can be taken into account during the acceptance of a message. For example, if \texttt{MCP2515} filters are defined with the \texttt{RXM0}, \texttt{RXF0}, \texttt{RXF1} registers, leaving \texttt{RXM1} equal to $0$ provides the transfer to \texttt{RXB1} of all messages discarded by \texttt{RXB0}.
...
{\bf Six filters.} For example, with four filters:
{ \small\begin{lstlisting}[language=c++]
  ACAN2515Settings settings (QUARTZ_FREQUENCY, 125 * 1000) ;
  const ACAN2515Mask rxm0 = extended2515Mask (0x1FFFFFFF) ;
  const ACAN2515Mask rxm1 = standard2515Mask (0x7FF, 0, 0) ;
...
The acceptance criterion is\footnote{See 4.5 DS20001801H, \emph{Message Acceptance Filters and Masks}, page 33.}:
...
This error occurs when the \texttt{mBitConfigurationClosedToDesiredRate} property of the \texttt{settings} object is \texttt{false}. This means that the \texttt{ACAN2515Settings} constructor cannot compute a CAN bit configuration close enough to the desired bit rate. For example:

{ \small\begin{lstlisting}[language=c++]
void setup () {
  ACAN2515Settings settings (QUARTZ_FREQUENCY, 1) ; // 1 bit/s !!!
  // Here, settings.mBitConfigurationClosedToDesiredRate is false
  const uint32_t errorCode = can.begin (settings) ;
  // Here, errorCode contains ACAN2515::kCANBitConfigurationTooFarFromWishedBitRate
}
\end{lstlisting}}
...
void setup () {
  ...
  Serial.print ("mBitConfigurationClosedToDesiredRate: ") ;
  Serial.println (settings.mBitConfigurationClosedToDesiredRate) ; // 1 (--> is true)
...
void setup () {
  ...
  Serial.print ("mBitConfigurationClosedToDesiredRate: ") ;
  Serial.println (settings.mBitConfigurationClosedToDesiredRate) ; // 0 (--> is false)
  Serial.print ("actual bit rate: ") ;
  Serial.println (settings.actualBitRate ()) ; //  842105 bit/s
  Serial.print ("distance: ") ;
  Serial.println (settings.ppmFromWishedBitRate ()) ; // 124 ppm
...
The second argument does not change the CAN bit computation, it only changes the acceptance test for setting the \texttt{mBitConfigurationClosedToDesiredRate} property. For example, you can specify that you want the computed actual bit to be exactly the wished bit rate:
{ \small\begin{lstlisting}[language=c++]
void setup () {
  Serial.begin (9600) ;
...
\begin{table}[!ht]
  ...
  \begin{tabular}{llllll}
    ...
    \texttt{mBitConfigurationClosedToDesiredRate} & \texttt{bool} & \texttt{true} & See \refSubsectionPage{CANbitSettings} \\
